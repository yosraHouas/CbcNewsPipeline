...
public class StoryRepository
{
    ...
    public async Task<List<Story>> GetLatestAsync(string? feed, int limit, CancellationToken ct = default)
    {
    ...
    public async Task<long> CountAsync(string? feed, CancellationToken ct = default)
    {
        var filter = string.IsNullOrWhiteSpace(feed)
            ? Builders<Story>.Filter.Empty
            : Builders<Story>.Filter.Eq(x => x.Feed, feed);
